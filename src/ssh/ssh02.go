...
func main() {
	...
	userFlag := flag.String("user", "caton.hao", "user to use on the remote machine")
	addrsFlag := flag.String("addrs", "192.168.8.88:22", "comma separated list of address to ssh into")
	filesFlag := flag.String("files", "/home/caton.hao/4.txt", "comma separated list of path where files to tail are found")
	...
	clients, doneClient, err := connectHost(addrs, &ssh.ClientConfig{
		...
		//Auth:            []ssh.AuthMethod{ssh.Password("nebula")},
